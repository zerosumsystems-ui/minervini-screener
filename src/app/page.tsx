...
function MinerviniScreener() {
  ...
  const handleRunScreener = async () => {
    if (!apiKey) {
      setToast("Please enter an API key");
      setTimeout(() => setToast(""), 2000);
      return;
    }
  ...
  const formatValue = (val: any, key: string) => {
    ...
    if (key === "rvol" || key === "atrRatio" || key === "volRatio")
      ...
      <div className="bg-gradient-to-b from-gray-900/80 to-transparent border-b border-gray-800">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                className="px-3 py-2 bg-gray-800 border border-gray-700 rounded text-sm text-gray-100 placeholder-gray-500 w-40 focus:outline-none focus:border-gray-600"
              />
              <button
                onClick={handleRunScreener}
                disabled={loading}
                className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-gray-900 font-semibold rounded transition disabled:opacity-50"
              >
          ...
          <div className="text-xs text-gray-500 font-mono">
            Ready \u2014 {tabCounts.breakout} breakouts \u00b7 {tabCounts.vcp} VCP \u00b7{" "}
            {tabCounts.monitor} monitoring
          </div>

          {toast && (
            <div className="mt-3 px-3 py-2 bg-blue-500/20 border border-blue-500/50 rounded text-blue-300 text-sm">
              {toast}
            </div>
          )}
        </div>
              ...
              <tr>
                {getTabColumns().map((col) => (
                  <th
                    ...
                    className={`px-4 py-3 text-left font-semibold text-gray-400 cursor-pointer hover:text-gray-200 transition ${col.width}`}
                  >
                    {col.label}
                    {sortColumn === col.key && (
                      <span className="ml-1">
                ...
                filteredData.map((stock) => (
                  ...
                  >
                    {getTabColumns().map((col) => {
                      ...
                      if (col.key === "boQuality")
                        cellClass += ` font-mono ${getGradeColor(val)}`;
                      if (
                        ["rvol", "atrRatio", "volRatio", "ttScore", "priorityScore"].includes(
                          col.key
                        )
                      )
                        cellClass += " font-mono text-right text-gray-300";
                      if (col.key === "bbSqueeze")
                        cellClass += ` font-mono text-center ${
                          val ? "text-emerald-400" : ""
                        }`;
                      if (
                        ["range10dPct", "pctBelowHigh"].includes(col.key)
                      )
                        cellClass += " font-mono text-right text-gray-300";
          ...
          <div className="mt-6 p-6 bg-gray-900 border border-gray-800 rounded-lg">
            <div className="grid grid-cols-3 gap-8">
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <div>
                    ...
                    <span className="text-gray-100 ml-2 font-mono font-semibold">
                  ...
                  <div>
                    ...
                    <span
                      className={`ml-2 font-mono font-semibold ${getRsColor(
                        selectedStock.rsPct
                      )}`}
                    >
                      {selectedStock.rsPct}
              ...
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <div>
                    ...
                    <span className="text-gray-100 ml-2 font-semibold capitalize">
                      {selectedStock.stage}
                    </span>
                  </div>
                  {selectedStock.stage === "breakout" && (
                    <div>
                      ...
                      <span
                        className={`ml-2 font-mono font-bold ${getGradeColor(
                          selectedStock.boQuality
                        )}`}
                      >
                        {selectedStock.boQuality}
                      </span>
                  ...
                  <div>
                    ...
                    <span className="text-gray-100 ml-2 font-mono">
                      {selectedStock.rvol.toFixed(2)}x
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-500">Priority:</span>
                    <span className="text-amber-400 ml-2 font-mono font-semibold">
                      {selectedStock.priorityScore}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <div>
                    <span className="text-gray-500">50MA:</span>
                    <span className="text-gray-100 ml-2 font-mono">
                      ${selectedStock.ma50.toFixed(2)}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-500">200MA:</span>
                    <span className="text-gray-100 ml-2 font-mono">
                      ${selectedStock.ma200.toFixed(2)}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-500">BB Squeeze:</span>
                    <span
                      className={`ml-2 ${
                        selectedStock.bbSqueeze
                          ? "text-emerald-400 font-semibold"
                          : "text-gray-500"
                      }`}
                    >
                      {selectedStock.bbSqueeze ? "Yes" : "No"}
                    </span>
                  </div>
                </div>
              </div>
